<div class="row margin-bottom-10 dados-vendedor dados-pessoa padding-top-30">                            
  <div class="col-xs-2 col-sm-2 col-lg-2 col-lg-2 avatar">
  ...
  <div class="col-xs-10 col-sm-10 col-lg-10 col-lg-10">
    <div class="row">                                    
      <div class="col-xs-3 col-sm-3 col-lg-3 col-lg-3 avaliacao-empresa-mensagens">
        <input value="{{$mensagem->usuarioDestino->empresa->avaliacao}}" class="avaliacao rating-loading">
      </div>                                                                        
      <div class="col-xs-9 col-sm-9 col-lg-9 col-lg-9 qualidade">
        <p><img src="{{assets('sistema/images/icons/quality.png')}}" /> Vendedor Platinum</p> 
      </div>                                                                        
    </div>
    <div class="row padding-top-10">                                
      ...
      <div class="col-xs-6 col-sm-6 col-lg-6 col-lg-6 botoes-acao-user">
        @if( $mensagem->usuarioOrigem->id != $usuario->id )
          @if( !$usuario->present()->contato($mensagem->usuarioOrigem) )
            <button class="add add-contato" data-id="{{$mensagem->id}}" data-url="{{route('sistema.dash.contato.adicionar', $mensagem->usuarioOrigem)}}"><i class="far fa-user"></i>+</button>
          @endif
        @else
          @if( !$usuario->present()->contato($mensagem->usuarioDestino) )
            <button class="add add-contato" data-id="{{$mensagem->id}}" data-url="{{route('sistema.dash.contato.adicionar', $mensagem->usuarioDestino)}}"><i class="far fa-user"></i>+</button>
          @endif
        @endif
        <button class="whatsapp"><i class="fab fa-whatsapp"></i> WhatsApp</button>
        <button class="bloquear"><i class="fas fa-ban"></i> Bloquear</button>
      </div>
    ...
    @if($msg->destino == $usuario->id )
      <div class="row mensagem-comprador margin-bottom-50">                            
        <div class="col-xs-1 col-sm-1 col-lg-1 col-lg-1 imagem-comprador">
          @if( $mensagem->usuarioDestino->id != $usuario->id )
            <img src="{{$mensagem->usuarioDestino->imagem ?? assets('sistema/images/avatar.png')}}" />
          @else
            <img src="{{$mensagem->usuarioOrigem->imagem ?? assets('sistema/images/avatar.png')}}" />
          @endif
        ...
        <div class="col-xs-11 col-sm-11 col-lg-11 col-lg-11 padding-left-5">
          <div class="row">
            ...
            @else
              <h5>{{$mensagem->usuarioDestino->fullName}} </h5>
            @endif
          </div>
          @if($msg->tipo == 0 )
...
@if( $mensagem->usuarioOrigem->id != $usuario->id )
  <form data-action="{{route('sistema.dash.vendedor.mensagens.mensagem.responder', $mensagem->id)}}" class="form-resposta" data-mensagem="{{$mensagem->id}}">
@else
  <form data-action="{{route('sistema.dash.comprador.mensagens.mensagem.responder', $mensagem->id)}}" class="form-resposta" data-mensagem="{{$mensagem->id}}">
@endif
